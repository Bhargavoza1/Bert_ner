...
def process_data():
    ...
    '''# it will encode string to number 
        ...
        array([1, 1, 2, 0])
    '''
    data_set.loc[:, "POS"] = _pos_encoder.fit_transform(data_set["POS"])
    data_set.loc[:, "Tag"] = _tag_encoder.fit_transform(data_set["Tag"])
    ...
    '''
    sentences = data_set.groupby("Sentence #")["Word"].apply(list).values
    pos = data_set.groupby("Sentence #")["POS"].apply(list).values
    tag = data_set.groupby("Sentence #")["Tag"].apply(list).values
